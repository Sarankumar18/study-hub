...
1. **Wrong base cases:** Off-by-one in `dp[0]` vs `dp[1]`—match the recurrence to the problem (0-indexed vs 1-indexed).
2. **Order of iteration:** In tabulation, iterate so you always compute what you depend on first (usually left-to-right).
3. **Integer overflow:** Use `long` when `dp[i]` can grow beyond `Integer.MAX_VALUE` (e.g., large counts).
4. **Sentinel for "impossible":** Use a value that can't be a real answer (e.g., `amount+1` for coin change, `n` for min jumps).
5. **Memoization base case:** Return base values before checking the memo—otherwise you might store wrong values.
6. **Space optimization:** Only collapse when `dp[i]` depends on a fixed few values (e.g., `i-1`, `i-2`); LIS-style needs the full array.
7. **String indices:** `dp[i]` often means "prefix of length i"; `s.substring(j, i)` gives `s[j..i-1]`.
