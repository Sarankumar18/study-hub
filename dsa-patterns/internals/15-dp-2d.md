...
> Build a 2D table where each cell uses answers you already figured out. Three flavors: grid paths (like walking on a map), comparing two strings (LCS, edit distance), or picking items to hit a target (coin change, subset sum). Very common in interviews.

## What Is This Pattern?

**2D Dynamic Programming** is like filling out a spreadsheet. Instead of a single row, you have rows and columns. Each cell `dp[i][j]` depends on cells you filled earlier. Think of it as: "I already know answers for smaller pieces—how do I combine them?"

Three main types:

1. **Grid DP:** You're on a grid (like a chessboard). `dp[r][c]` = best value to reach that cell. Fill row by row or column by column. Good for: counting paths, finding minimum cost paths, dealing with obstacles.
2. **Two-sequence DP:** You have two strings (or arrays). `dp[i][j]` = best result for the first i chars of one and first j chars of the other. Good for: longest common subsequence, edit distance, checking if one string matches a pattern.
3. **2D Knapsack:** You have items and a target (amount or capacity). `dp[i][j]` = result using first i items to reach sum/capacity j. Good for: counting coin combinations, subset sum.

This pattern works when: (1) the best overall answer needs best answers for smaller parts, and (2) you solve the same small parts many times. You can fill the table bottom-up or use memoization top-down—both work.

## When to Use

- The input is **two sequences** (strings, arrays) or a **grid**.
- You need the **longest, maximum, or minimum** of something involving both.
- The question asks to **count ways** or **check if something exists** with limits on two things.
- The answer naturally needs **two indices** (e.g., "first i chars of s1, first j chars of s2").
- Buzzwords: "longest common", "edit distance", "minimum path sum", "number of ways", "can s3 be formed by interleaving s1 and s2".
