...
- **Brute Force:** Recursively try all combinations: at each step, either use the current coin (if amount allows) or skip to next coin; count when amount reaches 0. Time O(2^n), Space O(n)
...
- **Brute Force:** Try all 2^n assignments of + or - to each number, count how many yield the target sum. Time O(2^n), Space O(n)
...
- **Brute Force:** Recursively try all operations (insert, delete, replace) at each position when chars differ; explore all paths. Time O(3^(m+n)), Space O(m+n)
